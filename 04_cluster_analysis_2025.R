...
df_cob_suplementar_25 <- df_cob_suplementar |>
  filter(ano %in% c(2025)) |>
  group_by(codmunres) |>
  summarise(
...
df_cluster <- df_cob_suplementar_25 |> select(cob_suplementar) |> scale()
...
rect.hclust(d1_ward, k = 4, border = 2:5) # Candidates: K = 3 and K = 4.
...
write.csv(d1_avaliacao, "r_objects/df_avaliacao_cluster_2025.csv", row.names = FALSE)

#### Comments:
##### db_index_cent: the lower the better (ward3, followed by ward4 and kmeans4)
##### db_index_med: the lower the better (ward3, followed by ward3 and kmeans3)
##### dunn_index: the higher the better (ward3, followed by ward4 and kmeans4)
##### silh_index: the higher the better (ward3, followed by kmeans3 and pam3)
##### ch_index_cent: the higher the better (kmeans4, followed by pam4 and ward4)
##### ch_index_med: the higher the better (kmeans4, followed by pam4 and ward4)

#### Choice: K-means with K = 4 to better compare with the other clusters
d1_kmeans4$size

### Adding the columns with the clusters informations to the original data.frames
df_cob_suplementar_25$cluster <- d1_kmeans4$cluster

ordem_taxa <- df_cob_suplementar_25 |>
  group_by(cluster) |>
...
df_cob_suplementar_25$cluster <- factor(case_when(
  df_cob_suplementar_25$cluster == ordem_taxa[1] ~ "1: Lowest percentage of women with private health insurance",
  df_cob_suplementar_25$cluster == ordem_taxa[2] ~ "2: Low percentage of women with private health insurance",
  df_cob_suplementar_25$cluster == ordem_taxa[3] ~ "3: Intermediate percentage of women with private health insurance",
  df_cob_suplementar_25$cluster == ordem_taxa[4] ~ "4: High percentage of women with private health insurance"
))
...
df_dados_mapa <- left_join(df_cob_suplementar_25, df_muni_sf) |>
  st_as_sf()
...
### Exporting the plot
saveRDS(plot_2025, "r_objects/plot_clusters_2025.rds")

### Joining the three maps
plot_2015 <- readRDS("r_objects/plot_clusters_2015.rds")
plot_2019 <- readRDS("r_objects/plot_clusters_2019.rds")
plot_2025 <- readRDS("r_objects/plot_clusters_2025.rds")
...
write.csv(tabela_regioes, "r_objects/df_regioes_clusters_2025.csv", row.names = FALSE)
...
df_cob_suplementar_indicadores <- left_join(df_cob_suplementar_25, df_indicadores)
...
tabela_resumo_cob
write.csv(tabela_resumo_cob, "r_objects/tabela_resumo_cluster_2025.csv", row.names = FALSE)

nomes_variaveis2 <- c("razao_emprego_formal_pop_15_a_64", "idhm", "porc_populacao_urbana", "porc_va_adespss", "porc_beneficiarios_bolsa_familia")
...
write.csv(tabela_resumo_indicadores, "r_objects/tabela_resumo_indicadores_2025.csv", row.names = FALSE)

### Creating the multiple comparisions tables
tabela_comparacoes_cob <- cria_tabelas_testes(
  df = df_cob_suplementar_25, var_grupos = "cluster", variaveis = nomes_variaveis1
)
tabela_comparacoes_cob
write.csv(tabela_comparacoes_cob, "r_objects/tabela_comparacoes_cluster_2025.csv", row.names = TRUE)
...
write.csv(tabela_comparacoes_indicadores, "r_objects/tabela_comparacoes_indicadores_2025.csv", row.names = TRUE)
