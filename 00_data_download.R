...
df_info_muni <- read.csv("databases/df_aux_municipios.csv")
...
df_aux_municipios <- data.frame(
  codmunres = rep(codigos_municipios, each = length(2015:2024)),
  ano = 2015:2024
) |>
...
df_est_pop <- est_pop_tabnet(periodo = 15:24, filtro_sexo = 2, idade_min = 10, idade_max = 49)
...
df_cob_suplementar <- df_beneficiarias_pop |>
  ...
  select(
    codmunres,
    ano,
    beneficiarias_10_a_49 = novo_beneficiarias_10_a_49,
    populacao_feminina_10_a_49
  )

### Juntando com a base auxiliar de municípios
df_beneficiarias_final <- left_join(
  df_aux_municipios,
  df_cob_suplementar |> mutate(codmunres = as.numeric(codmunres))
) |>
  arrange(codmunres, ano)

### Substituindo os NA's da coluna 'beneficiarias_10_a_49' por 0 (gerados após o left_join)
df_beneficiarias_final$beneficiarias_10_a_49[is.na(
  df_beneficiarias_final$beneficiarias_10_a_49)] <- 0

### Salvando a base final
write.csv(df_beneficiarias_final, "databases/df_beneficiarias_10_a_49.csv", row.names = FALSE)
