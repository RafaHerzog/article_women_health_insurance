...
df_cob_suplementar_15 <- df_cob_suplementar |>
...
df_cluster <- df_cob_suplementar_15 |> select(cob_suplementar) |> scale()
...
rect.hclust(d1_ward, k = 3, border = 2:5) # Candidates: K = 2, 3 and K = 4.
rect.hclust(d1_ward, k = 4, border = 2:5)
...
write.csv(d1_avaliacao, "r_objects/df_avaliacao_cluster_2015.csv", row.names = FALSE)

#### Comments:
##### db_index_cent: the lower the better (kmeans4, followed by kmeans3 and pam4)
##### db_index_med: the lower the better (kmeans4, followed by kmeans3 and pam4)
##### dunn_index: the higher the better (kmeans4, followed by ward4 and pam4)
##### silh_index: the higher the better (kmeans3, followed by kmeans4 and ward4)
...
df_cob_suplementar_15$cluster <- d1_kmeans4$cluster

ordem_taxa <- df_cob_suplementar_15 |>
  group_by(cluster) |>
...
df_cob_suplementar_15$cluster <- factor(case_when(
  df_cob_suplementar_15$cluster == ordem_taxa[1] ~ "1: Lowest percentage of women with private health insurance",
  df_cob_suplementar_15$cluster == ordem_taxa[2] ~ "2: Low percentage of women with private health insurance",
  df_cob_suplementar_15$cluster == ordem_taxa[3] ~ "3: Intermediate percentage of women with private health insurance",
  df_cob_suplementar_15$cluster == ordem_taxa[4] ~ "4: High percentage of women with private health insurance"
))


## Visualizing the clusters in a map ---------------------------------------
### Downloading the geometry data
df_muni_sf <- read_municipality(year = 2020, cache = FALSE) |>
  mutate(codmunres = as.numeric(substr(code_muni, 1, 6)))

df_ufs_sf <- read_state(year = 2020, cache = FALSE)

### Joining the geometry and the clustering data for each period
df_dados_mapa <- left_join(df_cob_suplementar_15, df_muni_sf) |>
  st_as_sf()
...
### Exporting the plot
saveRDS(plot_clusters, "r_objects/plot_clusters_2015.rds")

### Creating a table with the percentage of municipalities in each region
tabela_regioes <- df_cob_suplementar_15 |>
...
write.csv(tabela_regioes, "r_objects/df_regioes_clusters_2015.csv", row.names = FALSE)
...
df_cob_suplementar_indicadores <- left_join(df_cob_suplementar_15, df_indicadores)
...
tabela_resumo_cob
write.csv(tabela_resumo_cob, "r_objects/tabela_resumo_cluster_2015.csv", row.names = FALSE)
...
write.csv(tabela_resumo_indicadores, "r_objects/tabela_resumo_indicadores_2015.csv", row.names = FALSE)

### Creating the multiple comparisions tables
tabela_comparacoes_cob <- cria_tabelas_testes(
  df = df_cob_suplementar_15, var_grupos = "cluster", variaveis = nomes_variaveis1
)
tabela_comparacoes_cob
write.csv(tabela_comparacoes_cob, "r_objects/tabela_comparacoes_cluster_2015.csv", row.names = TRUE)
...
write.csv(tabela_comparacoes_indicadores, "r_objects/tabela_comparacoes_indicadores_2015.csv", row.names = TRUE)
