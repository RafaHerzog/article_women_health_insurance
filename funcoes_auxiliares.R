...
est_pop_tabnet <- function(periodo = 12:25, filtro_sexo = 2, idade_min = 10, idade_max = 49, temp_dir = "databases/dados_populacao") {
  library(dplyr)
  library(foreign)
  library(RCurl)
  library(utils)
  ...
  for (ano in anos) {
    ...
    df <- df |>
      ...
      filter(sexo == as.numeric(filtro_sexo), idade >= idade_min, idade <= idade_max)
